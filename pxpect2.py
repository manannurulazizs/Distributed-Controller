...
if __name__=="__main__":
    ...
    global pxpct
    pxpct = pexpect.spawn('python',['Topologi1.py'])
    pxpct.expect('mininet>')
    pxpct.sendline("py net.get('switch1').start([Controller1])")
    pxpct.expect('mininet>')
    pxpct.sendline("py net.get('switch2').start([Controller1])")
    pxpct.expect('mininet>')
    pxpct.sendline("py net.get('switch3').start([Controller1])")
    pxpct.expect('mininet>')
    pxpct.sendline("py net.get('switch4').start([Controller1])")
    pxpct.expect('mininet>')

    pxpct.sendline('xterm h1 h2')
    pxpct.expect('mininet>')
